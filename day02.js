const day02 = {
  /**
   * Takes an "spreadsheet" string whose rows are separated by newlines, and
   * whose cells are separated by tabs or spaces, and returns an array of arrays
   */
  formatSpreadsheet: function(string) {
    const splitByNewlines = string.split('\n')
    const splitBySpace = splitByNewlines.map((str) => {
      // First replace any tabs with spaces
      const newStr = str.replace(/\t/g, ' ')
      return newStr.split(' ')
    })
    const result = splitBySpace.map((array) => {
  ...
  calculateChecksumPartOne: function(matrix) {
    ...
    matrix.forEach((array) => {
      array.sort((a, b) => (a - b))
      const checksumNumber = array[array.length - 1] - array[0] 
      checksumNumbers.push(checksumNumber)
    })
  ...
  generateAnswer: function(string) {
    const puzzleInput = `3458	3471	163	1299	170	4200	2425	167	3636	4001	4162	115	2859	130	4075	4269
